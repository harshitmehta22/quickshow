          ...
          {booking.map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-col md:items-end md:text-right justify-between p-4">
                ...
                {!item.isPaid && <button className="bg-primary px-4 py-1.5 mb-3 text-sm rounded-full full-medium cursor-pointer">Pay Now</button>}
                </div>
                <div className="text-sm">
                    <p><span>Total Tickets:</span> {item.bookedSeats.length}</p>
                    <p><span>Seat Number:</span> {item.bookedSeats.join(', ')}</p>
                </div>
                <button className="px-3 py-1 bg-primary rounded text-white">View</button>
              </div>
            </div>
          ))}
        </div>
